...
class Login extends React.Component<LoginProps, LoginState>{
    ...
    handleSubmit = async (event: any) => {
        ...
        if (validity == true && this.state.user_name != '' && this.state.password != '') {
            const res = await signin({user_name: this.state.user_name, password: this.state.password} as LoginState) as Result;
            if (res.data) {
                ...
                <div className='wrapper'>
                    <div style={{ margin: "20px" }} className="col-sm-3">
                        <div className='form-wrapper'>
                            ...
                            <div className='submit'>
                                    <p className="submitparagraph">Or Dont Have an account ? <RouterLink to="/register" className="myLink" >Sign up</RouterLink></p>
                            </div>
                        </div>
                    </div>
                </div>
